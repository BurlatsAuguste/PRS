    ...
    while(NUM_SEG < len(Tab_ACK) and Last_Segment == False):
        ...
        for i in range(cwnd):
            if((i+NUM_SEG) > len(Tab_ACK)):
                break
            with Tab_ACK_lock:
                if((Tab_ACK[i+NUM_SEG - 1][1] == False) or ((Tab_ACK[i+NUM_SEG - 1][1] == True) and Tab_ACK[i+NUM_SEG - 2][0] > 1)):
                    print("send : ", i+NUM_SEG)
                    sock_client.sendto(Tab_ACK[i+NUM_SEG-1][2], address)
                    Tab_ACK[i+NUM_SEG-1][1] = True
                    print("ACKi-1 = ", Tab_ACK[i+NUM_SEG-1][0], " ", i+NUM_SEG-1)
